...
export default function Ressources() {
  ...
  useEffect(() => {
    if (selectedRessource) {
      ...
      setSelectedTechId(selectedRessource.technology.id);
  ...
  const handleUpdateRessource = async (id) => {
    ...
    setSelectedTechId(ressource.technology.id);
    ...
    <div className="ressources">
        ...
        >
          {filteredRessources.map((res) => (
            <div key={res.id}>
              ...
              <p>
                {res.technology.name} · {res.label} · {res.type}
              </p>

              <button onClick={() => handleUpdateRessource(res.id)}>
                Update
              </button>

              <button onClick={() => handleDeleteRessource(res.id)}>
                Delete
              </button>
            </div>
          ))}
      ...
      <Suspense fallback={<div>Loading...</div>}>
        <PopUpModal
          ...
          title={editRessources ? "udpate" : "add"}
        >
          <div className="ressource-modal">
            <form>
                  ...
                  >
                    ...
                    {category.map((category) => (
                      <option key={category.id} value={category.id}>
                        {category.name}
                        {!category.defaultCategory && category.type && (
              ...
              <button onClick={handleSubmitRessource}>
                {editRessources ? "udpate" : "add"}{" "}
              </button>
            </form>
          </div>
        </PopUpModal>
